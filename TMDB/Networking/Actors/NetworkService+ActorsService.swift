...
extension NetworkManager {
    ...
    func getSearchActors(pageNumber:Int, query:String, completion: @escaping (
        _ result: Swift.Result<APIResponse<Person>, NetworkError>,
        _ statusCode: StatusCode?
        ) -> Void) {
        print(pageNumber)
        provider.request(MultiTarget(ActorsService.search(page: pageNumber, query: query))) { (result) in
            switch result {
            case .success(let response):
                if (200...299 ~= response.statusCode) {
                    do {
